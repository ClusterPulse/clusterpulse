...
func protoToClusterMetrics(p *pb.ClusterMetrics) *types.ClusterMetrics {
	return &types.ClusterMetrics{
		...
		NodesNotReady:      int(p.NodesNotReady),
		Namespaces:         int(p.Namespaces),
		NamespaceList:      p.NamespaceList,
		Pods:               int(p.Pods),
		PodsRunning:        int(p.PodsRunning),
		PodsPending:        int(p.PodsPending),
		PodsFailed:         int(p.PodsFailed),
		CPUCapacity:        p.CpuCapacity,
		CPUAllocatable:     p.CpuAllocatable,
		CPURequested:       p.CpuRequested,
		CPUUsagePercent:    p.CpuUsagePercent,
		MemoryCapacity:     p.MemoryCapacity,
		MemoryAllocatable:  p.MemoryAllocatable,
		MemoryRequested:    p.MemoryRequested,
		...
		StatefulSets:       int(p.Statefulsets),
		DaemonSets:         int(p.Daemonsets),
	}
}
